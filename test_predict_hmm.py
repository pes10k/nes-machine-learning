...
files_to_test = []
test_path = os.path.join("generated", "hmm")
...
for hmm_depth in range(2,7):
	...
	log_name = "HMM_" + str(hmm_depth) + "_predicted_hmm.txt"
...
log_name = "bayes_net_predicted_hmm.txt"
log_full_path = os.path.join(log_path, log_name)
f = open(log_full_path, 'w')
index = 1
...
log_name = 'model_aver_predicted_hmm_scores.txt'
log_full_path = os.path.join(log_path, log_name)
f = open(log_full_path, 'w')
